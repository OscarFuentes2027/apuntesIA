        ...
        for evento in pygame.event.get():
            ...
            if evento.type == pygame.KEYDOWN:
                if evento.key == pygame.K_a:
                    ...
                    graficar_datos()  # Llama a la función para graficar datos
                elif evento.key == pygame.K_q:
                    print("Juego terminado. Datos recopilados:", datos_modelo)
                    pygame.quit()
                    exit()

# Función para graficar los datos en datos_modelo
def graficar_datos():
    if datos_modelo:
        velocidades, distancias, saltos = zip(*datos_modelo)  # Separar los datos en tres listas

        plt.figure(figsize=(10, 5))
        
        # Graficar velocidad de la bala
        plt.subplot(3, 1, 1)
        plt.plot(velocidades, label="Velocidad de la bala", color="blue")
        plt.ylabel("Velocidad")
        plt.legend()

        # Graficar distancia entre la bala y el jugador
        plt.subplot(3, 1, 2)
        plt.plot(distancias, label="Distancia al jugador", color="green")
        plt.ylabel("Distancia")
        plt.legend()

        # Graficar el salto (1 si saltó, 0 si no saltó)
        plt.subplot(3, 1, 3)
        plt.plot(saltos, label="Salto (1=Saltó, 0=No)", color="red")
        plt.ylabel("Salto")
        plt.legend()
